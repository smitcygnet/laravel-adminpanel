...
@section ('title', trans('labels.backend.samples.management'))

@section('page-header')
    <h1>{{ trans('labels.backend.samples.management') }}</h1>
@endsection

@section('content')
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.samples.management') }}</h3>
        ...
        <div class="box-body">
            <div class="table-responsive data-table-wrapper">
                <table id="samples-table" class="table table-condensed table-hover table-bordered">
                    <thead>
                        <tr>
                            <th>{{ trans('labels.backend.samples.table.id') }}</th>
                            <th>{{ trans('labels.backend.samples.table.first_name') }}</th>
				<th>{{ trans('labels.backend.samples.table.amountt') }}</th>
				<th>{{ trans('labels.backend.samples.table.level') }}</th>
...
@section('after-scripts')
    ...
    <script>
        ...
        $(function() {
            var dataTable = $('#samples-table').dataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.samples.get") }}',
                    type: 'post'
                },
                columns: [
                    {data: 'id', name: '{{config('module.samples.table')}}.id'},
                    {data: 'first_name', name: '{{config('module.samples.table')}}.first_name'},
				{data: 'amountt', name: '{{config('module.samples.table')}}.amountt'},
				{data: 'level', name: '{{config('module.samples.table')}}.level'},
				...
				{data: 'dateone', name: '{{config('module.samples.table')}}.dateone'},
				...
				{data: 'profile_img', name: '{{config('module.samples.table')}}.profile_img'},
                    {data: 'created_at', name: '{{config('module.samples.table')}}.created_at'},
