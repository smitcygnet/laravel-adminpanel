...
class Generator
{
    ...
    protected $repo_create_action;
    protected $repo_update_action;
    ...
    public function __construct()
    {
        ...
        $this->packages_path = 'packages'.DIRECTORY_SEPARATOR;
    }
    ...
    public function initialize($input,$rowData)
    {
        ...
        $this->repo_create_action      = !empty($rowData['repo_create_action']) ? $rowData['repo_create_action'] : '';
        $this->repo_update_action      = !empty($rowData['repo_update_action']) ? $rowData['repo_update_action'] : '';
    ...
    public function createRepository()
    {
        ...
        $replacements = [
                ...
                'dummy_create_action_data'      => $this->repo_create_action,
                'dummy_update_action_data'      => $this->repo_update_action,
    ...
    public function createMigration()
    {
        ...
        if (Schema::hasTable($table)) {
            ...
            $basepath = base_path(
             ...
             // CODE TO copy Migration file in package folder:
             $packageFile = base_path(
                $this->packages_path.$this->migration_path.DIRECTORY_SEPARATOR.trim($migrationFile[0]).".php"
             );
             copyInPackageFolder($packageFile,$fileContentNew);
             // CODE TO copy Migration file in package folder:
    ...
    public function copyInPackageFolder($packageFile,$content){
        if(!file_exists($packageFile))
            {
                $fh = fopen($packageFile, 'w');
            }
        file_put_contents($packageFile,implode("", $content));
    ...
    public function migrationMiddleContent($nl,$dbFileData){
        ...
        foreach($dbFileData as $data){
                ...
                $value     = $data['value'];
                $nullable  = ($data['nullable'] == 1)? "->nullable()" : '';
 ...
 public function createRowDataForNewFiles(array $input,$moduledata)
    {
        $files   = ['getForDataTableRepoHtml',  'viewIndexLabelHtml', 'viewIndexColumnHtml',
                    ...
                    'uploadFormImgFunctionCode','viewIndexSingleCheckboxHtml', ];

        $rowData = [ 'labels'     =>[], 'core_data'      =>[], 'index_label'=>[], 'index_column' =>[],
                     'form_view'  =>[], 'rules_string'   =>[], 'fields_string'      =>[],
                     'input_type' =>[], 'backend_custom' =>[], 'repo_update_action' =>[],
                     'getfordatatable_string' =>[], 'repo_construct_data'=>[],
                     'repo_create_action'     =>[], 'repo_protected_var' =>[],
        ];
            ...
            $singular_tc_model = strtolower(str_singular(title_case($input['model_name'])));
            ...
            foreach ($rowData['core_data'] as $data) {
                ...
                $FNCC   = $data['fieldlabel'];//$this->changeFieldCase($data['fieldname'],'camel');
                ...
                $rowData['repo_create_action']  = $this->getCreateUpdateActDataForRepo(
                                                    $rowData['core_data'],'create', $singular_tc_model
                                                );
                $rowData['repo_update_action']  = $this->getCreateUpdateActDataForRepo(
                                                    $rowData['core_data'],'update', $singular_tc_model
                                                );
         ...
         switch ($data['input_type']) {
             ...
             case 'textarea':
              ...
              case 'checkbox':
                 ...
                 $replacements['DummyFieldHTML']     =  $this->getReplacedViewFormHTML($replacements,$files[13]);
                 break;
              ...
              case 'file':
                 $replacements['DummyModuleName']  = strtolower($replacements['DummyModuleName']);
                 $replacements['DummyFieldType']   = 'text';
                 $replacements['DummyClass']       = 'form-control box-size';
                 $replacements['DummyExtraParams'] = '';
    ...
    public function getCreateUpdateActDataForRepo($coreData,$type,$model){
      $returnArr    = [];
      $returnString = '';
      foreach($coreData as $fielddata){
        ...
        if($fielddata['input_type'] == 'file'){
            ...
            if($type == 'update'){
               $returnArr[] = "\n"."\t\t\t\$this->removeImage(\$$model, '".$field."_path', '".$field."');\n";
            }
            $returnArr[] = "\$input['".$field."'] = \$this->uploadFormImg(\$input['".$field."'],'".$field."_path');
            }";
        }
      }
